sap.ui.define([
    ...
    return Controller.extend("goscm.crud.controller.Main", {
        onBeforeRendering: function () {
            this._datailEmployeeView = this.getView().byId("detailEmployeeView");
        },
        onInit: function () {
        ...
        showEmployeeDetails: function (category, nameEvent, path) {
            ...
            this.onReadODataIncidence(this._datailEmployeeView.getBindingContext("jsonEmployees").getObject().EmployeeID);

        },
        onSaveODataIncidence: function (channelId, eventId, data) {
            var oResourceBundle = this.getView().getModel("i18n").getResourceBundle();
            var employeeId = this._datailEmployeeView.getBindingContext("jsonEmployees").getObject().EmployeeID;
            var incidenceModel = this._datailEmployeeView.getModel("incidenceModel").getData();
        ...
        onReadODataIncidence: function (employeeID) {
            this.getView().getModel("incidenceModel").read("/IncidentsSet", {
                ...
                success: function (data) {
                    var incidenceModel = this._datailEmployeeView.getModel("incidenceModel");
                    incidenceModel.setData(data.results);
                    var tableIncidence = this._datailEmployeeView.byId("tableIncidence");
                    tableIncidence.removeAllContent();
                    for (var incidence in data.results) {
                        var newIncidence = sap.ui.xmlfragment("goscm.crud.fragment.NewIncidence", this._datailEmployeeView.getController());
                        this._datailEmployeeView.addDependent(newIncidence);
